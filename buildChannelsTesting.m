...
function channel = buildChannelsTesting(instances,stimVals,varargin)
channel = [];
% check arguments
if any(nargin == [0])
  help buildChannels
...
global trueError taoParam sigmaParam rhoParam myRealChannel trueOmega;
rho_0 = 0.1;
sigma_0 = 0.3;
...
mu = zeros(size(channelWeights,2),1);
trueTao = taoParam;
trueSigma = sigmaParam;
trueRho = rhoParam;
trueOmega = trueRho*trueTao'*trueTao + (1-trueRho)*diag(diag(trueTao'*trueTao))+trueSigma^2*myRealChannel.channelWeights'*myRealChannel.channelWeights; 
trueError = mvnrnd(mu,trueOmega,192);
...
global trueError myRealChannel;
%rho = 0;
omega = rho*tao'*tao + (1-rho)*diag(diag(tao'*tao))+sigma^2*channelWeights'*channelWeights; 
