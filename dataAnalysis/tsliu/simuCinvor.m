...
% ampIncRatio=[1 1.2 1.4 1.6 1.8 2.0 2.5 3];
switch scenario
    case 1
        ...
        for i=1:length(ampRatio)
            ...
            [channelPrefs thisChannel thisFit]=fitForwardModel(instancesTrain,e.stimVals,instancesTest,e.stimVals,m,'ampScaleModel');
    ...
    case 2
        ...
        for i=1:length(ampRatio)
            ...
            instancesTest=cellfun(@(x) mtimes(x,ampRatio(i)),instancesTest,'UniformOutput',false);
            [channelPrefs thisChannel thisFit]=fitForwardModel(instancesTrain,e.stimVals,instancesTest,e.stimVals,m,'noiseModel');
            allChannelTuning(i,:)=thisChannel;
            allChannelFit(i,:)=thisFit;
        end
    case 3
        ...
        kappa=[4 3 2 1];
        ampRatio=[1 1 1 1];
        noiseRatio=[0.1 0.1 0.1 0.1]*2.5;
        plotRatio=kappa;
        ...
        for i=1:length(kappa)
            ...
            instancesTest=generateBoldResp(m,e,noiseRatio(i));
            instancesTest=cellfun(@(x) mtimes(x,ampRatio(i)),instancesTest,'UniformOutput',false);
            [channelPrefs thisChannel thisFit]=fitForwardModel(instancesTrain,e.stimVals,instancesTest,e.stimVals,m,'kappaModel');
            allChannelTuning(i,:)=thisChannel;
            allChannelFit(i,:)=thisFit;
        end
end
...
% set(h,'name','Amplitude gain model');
subplot(3,2,1:2);
...
function [channelPrefs, channelTuning, chanFit]=fitForwardModel(instancesTrain,trainStimVals,instancesTest,testStimVals,m,figTitle, varargin)
