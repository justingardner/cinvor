...
docinvorOnce('decon2gIns','instanceMethod=deconv',['rebuild=',num2str(rebuild)]);
...
if s.rebuild
  ...
  elseif s.rebuild==2
    for i = 1:s.lvf.nROI
      s.lvf.roi{i}.instance = s.lvf.roi{i}.classify;
      s.rvf.roi{i}.instance = s.rvf.roi{i}.classify;
      s.lvf.roi{i} = rmfield(s.lvf.roi{i},'classify');
      s.rvf.roi{i} = rmfield(s.rvf.roi{i},'classify');
    end
    disp(['saving result: ', 'Anal/',s.saveName]);
    save(['Anal/',s.saveName], 's','-v7.3');
    return;
...
if s.rebuild==2
  if strcmp(s.instanceMethod,'mean')
    s.roi = getInstances(v,s.roi,s.stimvol,'blockLen=7','type=mean','n=inf',['r2cutoff=',s.r2cutoff]); %TSL:the first arg, v, is not really used but kept to make it backward compatible (see getCanonical)
  elseif strcmp(s.instanceMethod,'deconv')
    disp('here we go');
    keyboard
    s.roi = getInstances(v,s.roi,s.stimvol,'type=deconv','n=inf','canonicalType',canonicalType,['r2cutoff=',s.r2cutoff]);
  end
  return;
end
...
for iFold=1:numRuns
  ...
  if strcmp(s.instanceMethod,'mean')
    sTrain.roi = getInstances(v,sTrain.roi,sTrain.stimvol,'blockLen=7','type=mean','n=inf',['r2cutoff=',s.r2cutoff]); %TSL:the first arg, v, is not really used but kept to make it backward compatible (see getCanonical)
  elseif strcmp(s.instanceMethod,'deconv')
    sTrain.roi = getInstances(v,sTrain.roi,sTrain.stimvol,'type=deconv','n=inf','canonicalType',canonicalType,['r2cutoff=',s.r2cutoff]);
  end
  sTest.roi = getInstances(v,sTest.roi,sTest.stimvol,sTrain.roi,['r2cutoff=',s.r2cutoff]); %get instances for testing data, same call with training ROIs passed in
...
s.roi = loadROITSeries(v,roiList,[],[],'straightXform=1'); %straightXform=1 retrieves all voxels in the ROI without size-matching, which could delete useful voxels for MVPA purpose
s.nROI = length(roiList);
s.roi = getInstances(v,s.roi,s.stimvol,'blockLen=7','n=inf',['type=',s.instanceMethod],['canonicalType=',canonicalType],['r2cutoff=',s.r2cutoff]); %TSL:the first arg, v, is not really used but kept to make it backward compatible (see getCanonical)
