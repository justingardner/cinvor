...
thisStd = mean(mean(std_mat))/sqrt(N_trials);
FourierVariance = thisStd*thisStd*M;
...
%clf
plotVect = fftshift(observedVariance(1:M) - FourierVariance);
...
std_mat = zeros(M,N);
for i = 1:M
	std_mat(i,:) = std(s.lvf.roi{1}.instance.instances{i+8})/normConst;
end
thisStd = mean(mean(std_mat))/sqrt(N_trials);
FourierVariance = thisStd*thisStd*M;
...
plot(plotVect,'r');
hold on;
errorbar(1:M,plotVect,avgSTD*1.96,'r')
